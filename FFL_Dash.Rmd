...
# Range is farthest point (b/w min or max points for/against) plus 20%
mid_pt <- round(mean(scatter_df$`Cumulative Points For`), digits = 0)
low_range <- mid_pt - round(max(abs(c(max_PF, max_PF, min_PF, min_PA) - mid_pt))*1.2, digits = 0)
high_range <- mid_pt + round(max(abs(c(max_PF, max_PF, min_PF, min_PA) - mid_pt))*1.2, digits = 0)
