...
### TO DO ###
  # Plotly updates - highlight, spline, date slider? Not working with image at end point atm
  # Add DT table, currently not working
  # gt() table all text black? 
  # Add favicon for the dash
  # Add tab icons
...
FFL_Table %>%
  ...
  text_transform(
    ...
    fn = function(x) {
      local_image(
        filename = FFL_Table[FFL_Table$Week == max(FFL_Table$Week), ]$Image,
        height = 25)
    }) %>%
  tab_header(
...
FFL_Table %>%
  ...
  geom_hline(yintercept = sum(FFL_Table$`Points For`)/(12*parse_number(max(FFL_Table$Week))),
             color = "#5f6161", size = 1) +
  geom_boxplot(alpha = 1) + 
  scale_fill_manual(values = ffl_pal) +
  coord_flip() +
  theme_classic() +
  labs(title = "Points Boxplot", subtitle = "[Through Week 8]") +
  guides(fill = "none") +
  theme(legend.position = "none",
...
What_If <- function(team, schedule) {
  ...
  cbind.data.frame(
    ...
    mutate(Win = ifelse(Team_Pts > Schedule_Pts, 1, 
                        ifelse(Team_Pts == Schedule_Pts, NA, 0))) %>%
    drop_na(Win) %>%
...
dt_table <- DT::datatable(FFL_Table %>% select(-Win, -Image) %>% arrange(Week, Position), 
                      rownames = FALSE, filter = 'top', options = list(pageLength = 12, autowidth = TRUE))
...
```{r AP Tiles, include=FALSE}

AP_Tiles <- function(team) {
  
  team_pts <- rep(FFL_Table %>% 
                    filter(Team == team) %>% 
                    .$`Points For`,
                  each = 11)   
  ...
  FFL_Table %>%
    filter(Team != team) %>% 
    cbind.data.frame("Team Points" = team_pts) %>%
    mutate(Difference = round(`Team Points` - `Points For`, digits = 1),
           WL = ifelse(Difference > 0, "Win", "Loss"),
           Focus = team) %>%
    select(Week, Team, Difference, WL, Focus)
  
}


AP_Tile_DF <- map(.x = unique(FFL_Table$Team), .f = AP_Tiles) %>%
  bind_rows() 

AP_Tile_DF %>%
  ggplot(aes(x = Week, y = Team, fill = WL)) +
  geom_tile() +
  scale_fill_manual(values = c("#eb633d", "#0bb83c")) +
  facet_wrap(~Focus) +
  theme_dark() +
